    ...
    public sealed class InputDevice : MidiDevice, IInputDevice
    {
        ...
        private void OnShortMessage(int message)
        {
            try
            {
                ...
                midiEvent.eNote = _channelParametersBuffer[0];
                midiEvent.eVelocity = _channelParametersBuffer[1];
                OnEventReceived(midiEvent);
            }
            catch (Exception ex)
            {
        ...
        private void OnSysExMessage(IntPtr sysExHeaderPointer)
        {
            ...
            try
            {
                ...
                midiEvent.eData = new byte[data.Length];
                Array.Copy(data, midiEvent.eData, data.Length);
                OnEventReceived(midiEvent);
